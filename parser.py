...
def lex_curlybraces(current, content):
  token = []
  block=True
  token.extend(["{","{"])
  current = current + 2
...
def lexer(content):
  ...
  while True:
    ...
    if p == "\'" or p == "\"":
       ...
       current, t = lex_curlybraces(current, content)
       tokens.append(t)
       continue
    elif p == " ":
...
def parser(tokens):
  ...
  parameter_rx = re.compile(r"\s*(?P<name>[$-\.A-Za-z0-9]+)(\s+(?P<type>\w+))?\s*")
